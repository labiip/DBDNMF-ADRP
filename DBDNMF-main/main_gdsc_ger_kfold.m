...
for cv_run = 1:1
    ...
    K_fold = 10;
    indices = crossvalind('Kfold',size(Known_index,1),K_fold);
...
%     Y_tmp = Y_tmp / max_Y_tmp;
    ...
    % DMF setup
    s=[r 260 m];% input size, hidden size 1, ..., output size
    ...
    k = 0.2;
    [Y_DMF,NN_MF]=DBDNMF(Y_tmp',Mask_test',s,options,k);
...
%     subplot(2,2,4);plot(1:length(obj_rec),obj_rec);
    ...
    global_err = sqrt(sum((Y_0(Known_index)-Y_recover1(Known_index)).^2)/length(~isnan(Y_0(Known_index))));
    

    num = Y_0;
    numpred = Y_recover1;
    
    %writematrix(Y_recover1,'gdsc_Y_recover1.csv')
...
for d = 1:size(num,1)
    ...
    disp(sprintf('global_pcc%d,ave_err:%d,ave_err_sr:%d,ave_pcc:%d,ave_pcc_sr:%d',global_pcc,ave_err,ave_err_sr,ave_pcc,ave_pcc_sr));
